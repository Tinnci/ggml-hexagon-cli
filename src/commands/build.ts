...
export async function buildAction(options: {
    ...
    if (options.fatalWarnings) cmakeArgs.push('-DLLAMA_FATAL_WARNINGS=ON');
    if (options.sanitizeThread) cmakeArgs.push('-DLLAMA_SANITIZE_THREAD=ON');
    if (options.sanitizeAddress) cmakeArgs.push('-DLLAMA_SANITIZE_ADDRESS=ON');
    if (options.sanitizeUndefined) cmakeArgs.push('-DLLAMA_SANITIZE_UNDEFINED=ON');
    if (options.buildTests) cmakeArgs.push('-DLLAMA_BUILD_TESTS=ON');
    if (options.buildTools) cmakeArgs.push('-DLLAMA_BUILD_TOOLS=ON');
    if (options.buildExamples) cmakeArgs.push('-DLLAMA_BUILD_EXAMPLES=ON');
    if (options.buildServer) cmakeArgs.push('-DLLAMA_BUILD_SERVER=ON');
